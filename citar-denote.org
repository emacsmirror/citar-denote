...
In Citar-Denote, the citation key is part of the front matter of Denote files through the =reference= keyword.

Citar-Denote stores the relationship between citation keys and notes in a cache. The package regenerates this cache after you edit a bibliography file or call any Citar functions. Each bibliographic note is marked with the configurable =bib= file tag (~citar-denote-keyword~). This tag reduces the search space to generate the cache.
...
When the literature has an electronic file registered and the ~citar-denote-open-attachment~ variable is ~non-nil~, then the first registered file opens in another window when creating a new note. Customise this variable to change this behaviour.
...
** Find unmatched references and citations
The ~citar-denote-nobib~ function lists all references and citations in your Denote collection that are absent in the global bibliography and local bibliographies in the =*Messages*= buffer.

This function first searches for all local bibliographies in Org files with the `citar-denote-keyword`. A local bibliography is indicated with the ~#+bibliography:"~ keyword.

The output of this function is a list of references and citation keys used in Denote that need to be added or corrected.
